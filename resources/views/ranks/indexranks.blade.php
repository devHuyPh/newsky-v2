            ...
            <div class="d-flex justify-content-between align-items-center px-4 flex-wrap">
                ...
                <div class="d-flex align-items-center gap-12 flex-wrap">
                    <div class="d-flex align-items-center gap-12 me-5" style="max-width: 250px">
                        <h5 class="mb-0 text-dark">{{ trans('core/base::layouts.sharing_day') }}</h5>
                    ...
                    <thead class="table-dark sticky-top">
                        <tr>
                            ...
                            <th class="py-3 px-4 d-none d-md-table-cell">
                                {{ trans('core/base::layouts.upgrade_conditions') }}</th>
                            <th class="py-3 px-4 d-none d-md-table-cell">
                                {{ trans('core/base::layouts.demotion_conditions') }}</th>
                    ...
                    <tbody>
                        @forelse($data as $item)
                            <tr data-id="{{ $item->id }}">
                                ...
                                <td class="px-4 py-3">
                                    <span class="badge bg-primary text-light px-2 py-1">{{ $item->rank_lavel }}</span>
                                </td>
                                ...
                                <td class="px-4 py-3 text-danger d-none d-md-table-cell">
                                    ...
                                    {{ number_format($item->demotion_investment, 0, ',', '.') }} VND
                                </td>
        ...
        <div class="card m-0 shadow-lg h-100" style="border-radius: 0;">
            <div class="row card-header bg-gradient-primary text-white py-3">
                <div class="d-flex justify-content-between align-items-center px-4 flex-wrap">
                    ...
                    <div class="d-flex align-items-center gap-12 flex-wrap">
                        <button class="btn btn-primary mt-2 mt-md-0" data-bs-toggle="modal" data-bs-target="#addRankModal">
                            <i class="fas fa-plus me-2"></i>{{ trans('core/base::layouts.add_new') }}
                        </button>
                    </div>
                </div>
                    ...
                    <table class="table table-hover table-striped align-middle m-0">
                        <thead class="table-dark sticky-top">
                            <tr>
                                ...
                                <th class="py-3 px-4">{{ trans('core/base::layouts.rank_name') }}</th>
                                <th class="py-3 px-4">{{ trans('core/base::layouts.level') }}</th>
                                <th class="py-3 px-4 d-none d-lg-table-cell">{{ trans('core/base::layouts.icon_rank') }}
                                </th>
                        ...
                        <tbody>
                            @forelse($customers as $customer)
                                <tr data-id="{{ $customer->id }}">
                                    ...
                                    <td class="px-4 py-3">{{ number_format($customer->total_dowline, 0, ',', '.') }}</td>
                                    <td class="px-4 py-3">{{ number_format($customer->walet_1, 0, ',', '.') }} VND</td>
                                    <td class="px-4 py-3">
                                    ...
                                    <td class="px-4 py-3">
                                        ...
                                        <button class="btn btn-danger btn-sm delete-customer-btn" data-bs-toggle="modal"
                            ...
                            <div class="form-group mb-3">
                                ...
                                <select name="rank_id" id="rank_id_add" class="form-control">
                                    ...
                                    @foreach($ranks as $rank)
                                        <option value="{{ $rank->id }}">{{ $rank->rank_name }} (Cấp {{ $rank->rank_lavel }})
                                        </option>
                                    @endforeach
                                </select>
                            </div>
        ...
        <div class="modal fade" id="deleteCustomerModal" tabindex="-1" aria-labelledby="deleteCustomerModalLabel"
            ...
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content shadow-lg">
                    ...
                    <div class="modal-body">Bạn có chắc chắn muốn xóa người dùng này khỏi danh sách không?</div>
                    <div class="modal-footer">
...
@push('js')
    <script>
        $(document).ready(function () {
            ...
            // Gán data-label cho bảng Ranking
            ...
            // Gán data-label cho bảng Customer
            $('.table').last().find('tbody tr').each(function () {
                $(this).find('td').each(function (index) {
                    const labels = ['Tên người dùng', 'Tên hạng', 'Cấp độ', 'Biểu tượng',
                        'Tổng tuyến dưới', 'Ví 1', 'Trạng thái Admin', 'Hành động'];
                    $(this).attr('data-label', labels[index]);
                });
            });
            ...
            $('.update-all-dayofsharing').on('click', function () {
                ...
                if (confirm('Are you sure you want to update Day of Sharing?')) {
                    $.ajax({
                        ...
                        method: 'POST',
                        data: { _token: '{{ csrf_token() }}', value: newValue },
                        beforeSend: function () {
                            $('.update-all-dayofsharing').prop('disabled', true);
                            $('.update-all-dayofsharing').html('<i class="fas fa-spinner fa-spin"></i>');
                        },
            ...
            $('.edit-rank-btn').on('click', function () {
                ...
                const form = $('#editRankForm');
                $('#customer_id').val(customerId);
                $('#rank_id').val(rankId || '');
                form.attr('action', '{{ url("/admin/customers") }}/' + customerId + '/update-rank');
            });
        });
    </script>
@endpush
