                    ...
                    <div class="d-flex align-items-center gap-12 me-5" style="max-width: 250px">
                        ...
                        <div class="input-group" style="max-width: 220px; overflow: hidden;">
                            <input type="number" class="form-control dayofsharing-input" max="31" min="0"
                                placeholder="Day of Sharing"
                                id="global-dayofsharing"
                                value="{{ $dayofsharing }}"
                ...
                <table class="table table-hover table-striped align-middle m-0">
                    <thead class="table-dark sticky-top">
                        <tr>
                            ...
                            <th class="py-3 px-4 d-none d-md-table-cell">{{ trans('core/base::layouts.upgrade_conditions')}}</th>
                            <th class="py-3 px-4 d-none d-md-table-cell">{{ trans('core/base::layouts.demotion_conditions')}}</th>
                    ...
                    <tbody>
                        @forelse($data as $item)
                            <tr data-id="{{ $item->id }}">
                                ...
                                <td class="px-4 py-3"><span class="badge bg-primary text-light px-2 py-1">{{ $item->rank_lavel }}</span></td>
                                ...
                                <td class="px-4 py-3 text-success d-none d-md-table-cell">
                                    <strong>{{ trans('core/base::layouts.referrals')}}</strong> {{ $item->number_referrals }}</br>
                                ...
                                <td class="px-4 py-3 text-danger d-none d-md-table-cell">
                                    <strong>{{ trans('core/base::layouts.months')}}</strong> {{ $item->demotion_time_months }}</br>
                                    <strong>{{ trans('core/base::layouts.referrals')}}</strong> {{ $item->demotion_referrals }}</br>
                                ...
                                <td class="px-4 py-3">
                                    <span class="badge {{ $item->status ? 'bg-success text-light' : 'bg-danger text-light' }} px-2 py-1">
                        ...
                        @empty
                            <tr>
                                <td colspan="9" class="text-center py-5 text-muted">
                                    <i class="fas fa-exclamation-circle me-2"></i>{{ trans('core/base::layouts.no_data_founds')}}
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
    ...
    <div class="modal fade" id="delete-modal" tabindex="-1" aria-labelledby="deleteModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content shadow-lg">
                ...
                <div class="modal-body">{{ trans('core/base::layouts.are_you_sure_you_want_to_delete_this_rank')}}</div>
        ...
        $(document).ready(function() {
            ...
            $('tbody tr').each(function() {
                $(this).find('td').each(function(index) {
                    const labels = ['Rank Name', 'Level', 'Icon', 'Upgrade Conditions', 'Demotion Conditions', 'Reward Percentage', 'Details', 'Status', 'Actions'];
