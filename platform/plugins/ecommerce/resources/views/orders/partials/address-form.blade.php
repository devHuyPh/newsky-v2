...
<script>
    document.addEventListener("DOMContentLoaded", function () {
        function getWardId() {
            ...
            document.addEventListener("DOMContentLoaded", function () {
                const districtId = citySelect.value;
                fetchWards(districtId);
            });

            citySelect.addEventListener("change", function () {
                const districtId = this.value; // Lấy district_id mới
                fetchWards(districtId);
            });
        }
    ...
    $(document).ready(function () {
        function updateFullAddress() {
            let addressDetail = $("#address_address_detail").val().trim();
            let addressWard = $("#address_ward option:selected").text().trim();

            if (addressDetail && addressWard) {
                $("#address_address").val(addressDetail + ", " + addressWard);
            } else {
                $("#address_address").val(addressDetail || addressWard);
            }
        }
        ...
        $("#address_address_detail, #address_ward").on("input change", updateFullAddress);

        // Gọi hàm khi trang tải xong để cập nhật sẵn giá trị nếu có
        updateFullAddress();
    });
