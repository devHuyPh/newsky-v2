    ...
    public function confirmPayment(Order $order): bool
    {
        ...
        if (!empty($user->getAttributes())) {
            ...
            CustomerNotification::create([
                'title' => 'Thông báo tổng thu nhập tuyến dưới',
                'dessription' => 'Bạn đã thanh toán đơn hàng trị giá ' . format_price($orderAmount) . '. Tổng thu nhập tuyến dưới là: ' . format_price($user->total_dowline) ,
                'customer_id' => $user->id,
            ...
            if($user->rank_assigned_at != null){
                $user->update([
                    'total_dowline_on_rank' => (float) $user->total_dowline + $orderAmount
                ]);
            }
            ...
            while ($referrer) { // Lặp đến người giới thiệu cuối cùng
                ...
                CustomerNotification::create([
                    'title' => 'Thông báo tổng thu nhập tuyến dưới',
                    'dessription' => 'F'.$level .' của bạn đã thanh toán đơn hàng trị giá: ' . format_price($orderAmount) . '. Tổng thu nhập tuyến dưới là: ' . format_price($referrer->total_dowline) ,
                    'customer_id' => $referrer->id,
                    'url' => '/bitsgold/dashboard'
                ]);
                $referrer->updateRank();
                $referrer->updateTotalRevenue();

                if($referrer->rank_assigned_at != null){
                    $referrer->update([
                        'total_dowline_on_rank' => (float) $referrer->total_dowline + $orderAmount
                    ]);
                }
                ...
                if ($commission >= 10000) {
                    $referrer->update([
                        'walet_1' => (float) $referrer->walet_1 + $commission
                    ]);

                    $referrer->refresh();

                    CustomerNotification::create([
                        'title' => 'Thông báo hoa hồng cho đơn hàng',
                        'dessription' => 'F'.$level .' của bạn đã thanh toán đơn hàng trị giá: ' . format_price($orderAmount) . '. Ví thu nhập của bạn là: ' . format_price($referrer->walet_1) ,
                        'customer_id' => $referrer->id,
                        'url' => '/bitsgold/dashboard'
                    ]);
                }
